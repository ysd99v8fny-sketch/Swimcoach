...
// matters more than the whole-session average pace, both for zone
// calibration and for estimating Critical Swim Speed / training load. That's
// one extra Strava call per swim, which on a big history can hit Strava's
// 15-minute rate limit in a single run. This is handled by:
//   - Skipping the lap fetch for any swim that already has laps stored from
//     a previous sync, so re-running only costs requests for genuinely new
//     activities.
...
//
// Genuinely new sessions (activity ids not already in storage) also get a
// short automatic coach comment from Claude — see lib/coachComment.js.
// Deliberately scoped to new sessions only, never re-generated for sessions
// that were already synced before, so re-running sync doesn't burn API
// credits on your existing history.
//
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Activities already stored at all — used to scope the coach comment to
    // genuinely new sessions only (see the comment on the handler above).
    ...
    outer: while (true) {
      ...
      const pageSessions = await mapWithConcurrency(swims, LAP_FETCH_CONCURRENCY, async (a, _i, stop) => {
        ...
        const isNewSession = !existingIds.has(session.id);
        ...
        // Only genuinely new sessions get a comment — never re-generated for
        // sessions that were already in storage before this run.
        if (isNewSession) {
          const comment = await generateCoachComment(session, existing);
          if (comment) {
            session.coachComment = comment;
            commentsGenerated += 1;
          }
        }
        return session;
