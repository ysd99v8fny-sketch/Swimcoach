...
// `lapPaces` (seconds/100m per lap) — see lib/strava.js getLapPaces for why
// this matters more than the whole-session average pace. That's one extra
// Strava call per swim, which on a big history can hit Strava's 15-minute
// rate limit in a single run. This is handled by:
//   - Skipping the lap fetch for any swim that already has lapPaces stored
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const idsWithLaps = new Set(
      existing.filter((s) => Array.isArray(s.lapPaces) && s.lapPaces.length > 0).map((s) => s.id)
    );
    ...
    outer: while (true) {
      ...
      const pageSessions = await mapWithConcurrency(swims, LAP_FETCH_CONCURRENCY, async (a, _i, stop) => {
        ...
        if (idsWithLaps.has(session.id)) {
          ...
          session.lapPaces = prior?.lapPaces || [];
          return session;
        }
        try {
          const { paces, rateLimitClose } = await getLapPaces(a.id, token);
